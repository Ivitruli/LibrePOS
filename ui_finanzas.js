...
window.renderCuentas = function() { document.getElementById('lista-cuentas').innerHTML = store.db.cuentas.map(c => `<div class="account-card"><div class="account-name">${c.nombre}</div><div class="account-bal">${fmt(finanzas.calcSaldoCuenta(c.id))}</div><div style="display:flex;gap:.3rem;margin-top:.5rem;"><input type="number" placeholder="Saldo Real" id="real-${c.id}" style="padding:.3rem;font-size:.8rem;"><button class="btn btn-secondary btn-sm" onclick="window.ajustarCaja('${c.id}', document.getElementById('real-${c.id}'))">Ajustar</button></div></div>`).join(''); };
