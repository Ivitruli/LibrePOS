    ...
    c.innerHTML = store.db.promociones.map(p => `
        <div style="background:var(--surface); border:1px solid var(--border); border-radius:8px; padding:1rem; display:flex; flex-direction:column; justify-content:space-between;">
            ...
            <div style="margin-top:1rem; display:flex; justify-content:space-between; align-items:flex-end; border-top:1px dashed var(--border); padding-top:.5rem;">
                ...
                <button class="btn btn-sm btn-danger" onclick="if(confirm('¿Eliminar promoción?')){ store.db.promociones = store.db.promociones.filter(x=>x.id!=='${p.id}'); store.saveDB(); window.renderPromosActivas(); if(typeof window.renderPromosActivasPOS === 'function') window.renderPromosActivasPOS(); }">🗑️ Borrar</button>
            </div>
        </div>
    `).join('');
};
...
window.registrarGasto = function() { try { finanzas.registrarGasto(document.getElementById('gasto-fecha').value, document.getElementById('gasto-cat').value, document.getElementById('gasto-tipo').value, document.getElementById('gasto-importe').value, document.getElementById('gasto-cuenta').value, document.getElementById('gasto-desc').value); store.saveDB(); window.renderTablaGastos(); window.renderFinanzasTotales(); window.renderCuentas(); window.showToast('Gasto ok'); } catch(e) { window.showToast(e.message, 'error'); } };
...
window.registrarMovimientoSocio = function() { try { socios.registrarMovimiento(document.getElementById('mov-socio').value, document.getElementById('mov-tipo').value, document.getElementById('mov-importe').value, document.getElementById('mov-cuenta').value, document.getElementById('mov-fecha').value); store.saveDB(); window.renderSocios(); window.renderFinanzasTotales(); window.renderCuentas(); window.showToast('Registrado'); } catch(e) { window.showToast(e.message, 'error'); } };
